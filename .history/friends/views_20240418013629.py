...
class GetSuggestionFriendView(APIView):
        def get(self, request):
            ...
            sent_friend_requests_id = FriendRequest.objects.filter(from_id=user).values_list('to_id', flat=True)
            received_friend_requests_id = FriendRequest.objects.filter(to_id=user).values_list('from_id', flat=True)
            
            other_users = User.objects.exclude(
            Q(email=user)| 
            Q(id__in=sent_friend_requests_id.values_list('id', flat=True)) |
            Q(id__in=received_friend_requests_id.values_list('id', flat=True))
            )
