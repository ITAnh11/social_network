...
class GetSuggestionFriendView(APIView):
        def get(self, request):
            ...
            received_friend_requests = FriendRequest.objects.filter(to_id=user).values_list('from_id', flat=True)
            #other_users = FriendRequest.objects.filter(id=sent_friend_requests.values_list('id', flat=True))
            print(received_friend_requests)
